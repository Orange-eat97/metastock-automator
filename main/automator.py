...
def run_add_request(request: AddExplorerRequest) -> None:
    """
    Legacy name for create-only behavior.

    Boundary:
        create Explorer only.
        no selection.
        no execution.
    """
    create_explorer_request(request)


def create_explorer_request(request: AddExplorerRequest) -> None:
    """
    Create an Explorer in MetaStock.

    This function intentionally does not select or run the Explorer.
    """
    actions, selectors, app, console = build_shared_components()
...
def select_explorer_request(request: AddExplorerRequest):
    """
    Select an existing Explorer and instruments.

    This function intentionally does not create or run the Explorer.
    """
    workflow = build_workflow(
        max_execution_wait_sec=request.max_execution_wait_sec,
    )
    return workflow.select_existing_explorer(request)


def run_selected_explorer_request(request: AddExplorerRequest):
    """
    Run the currently selected Explorer.

    This function intentionally does not create or select an Explorer.
    It clicks Start, waits for completion, and leaves the result window open.
    """
    workflow = build_workflow(
        max_execution_wait_sec=request.max_execution_wait_sec,
    )
    return workflow.run_selected_until_results_ready(request)


def run_existing_explorer_and_wait_request(request: AddExplorerRequest):
    """
    Legacy convenience wrapper: select existing Explorer, then run selected.

    Agent-facing service methods should call select_explorer_request() and
    run_selected_explorer_request() separately instead of using this wrapper.
    """
    select_explorer_request(request)
    return run_selected_explorer_request(request)


def run_add_and_wait_request(request: AddExplorerRequest):
    """
    Legacy convenience wrapper: create, select, then run.

    Agent-facing service methods should call create_explorer_request(),
    select_explorer_request(), and run_selected_explorer_request() separately
...
def run_add_and_run_request(request: AddExplorerRequest):
    """
    Backward-compatible combined CLI/GUI behavior:
    create -> select -> run -> capture -> verify -> close.
    """
    create_explorer_request(request)
